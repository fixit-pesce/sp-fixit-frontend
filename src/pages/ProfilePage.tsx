  ...
  return (
    <BaseLayout>
      <Flex justifyContent="center" alignItems="center" h = "calc(100vh - 72px)">
        {data && <Box bg = "white" boxShadow="md" rounded = "md" p = "8">
          <Heading pb = "4">Profile details - {sp_username}</Heading>
          <FormControl>
            <FormLabel>Company Name</FormLabel>
            <EditableTextInput defaultValue={data.company_name} setText = {setCompanyName} type = "input" />
          </FormControl>
          <Spacer h = "4"/>
          <FormControl>
            <FormLabel>Email</FormLabel>
            <EditableTextInput defaultValue={data.email} setText = {setEmail} type = "input" />
          </FormControl>
          <Flex pt = "4" justifyContent="center" gap = "4">
            <Button color = "white" bg = "secondary.400" _hover = {{bg: "secondary.500"}}>Save</Button>
            <Button color = "white" bg = "primary.400" _hover = {{bg: "primary.500"}}>Change Password</Button>
            <Button colorScheme="red" onClick = {onDeleteOpen}>Delete Account</Button>
          </Flex>
        </Box>}
        <DeleteProfileModal isOpen = {isDeleteOpen} onClose = {onDeleteClose} sp_username={sp_username}/>
      </Flex>
    </BaseLayout>
  )
}